...
function ganNoiDungTroLyAnToan(phanTu, html) {
    const mau = document.createElement("template");
    mau.innerHTML = String(html || "");
    const theChoPhep = new Set([
        "ARTICLE", "BR", "BUTTON", "DIV", "P", "SECTION", "SMALL", "SPAN", "STRONG"
    ]);
    [...mau.content.querySelectorAll("*")].forEach((the) => {
        if (!theChoPhep.has(the.tagName)) {
            the.replaceWith(document.createTextNode(the.textContent || ""));
            return;
        }
        [...the.attributes].forEach((thuocTinh) => {
            const laClass = thuocTinh.name === "class";
            const laMaLop = the.tagName === "BUTTON" && thuocTinh.name === "data-ai-course";
            if (!laClass && !laMaLop) the.removeAttribute(thuocTinh.name);
        });
        if (the.tagName === "BUTTON") the.type = "button";
    });
    phanTu.replaceChildren(mau.content.cloneNode(true));
}
...
function themTinNhanAI(noiDung, vaiTro = "assistant", laHtml = false) {
    ...
    if (laHtml) ganNoiDungTroLyAnToan(tinNhan, noiDung);
